...
export function SubtaskList({ parentTaskId }: SubtaskListProps) {
  ...
  const completedCount = subtasks.filter(t => t.completedAt).length
        ...
        {subtasks.map(task => (
          <div key={task.id} className="flex items-center gap-2 group hover:bg-muted/40 px-1.5 py-1 rounded transition-colors text-xs">
            <Checkbox 
              ...
              onCheckedChange={() => toggleComplete(task.id, !!task.completedAt)}
              className="h-3.5 w-3.5 rounded-[3px]"
            />
            <span className={`flex-1 text-xs truncate ${task.completedAt ? 'line-through text-muted-foreground/80' : 'text-foreground'}`}>
              {task.title}
            </span>
            <Button 
              variant="ghost" 
